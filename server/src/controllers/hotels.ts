  ...
  create: async (data: CreateHotelRequest, createdBy: string): Promise<Hotel> => {
    ...
    const newHotel: Hotel = {
      ...
      roomTypes: data.roomTypes.map(rt => ({
        ...
        id: generateId(),
        stock: rt.stock ?? 10,
        status: rt.status ?? 'available',
        bedType: rt.bedType ?? (rt as any).beds,
        maxOccupancy: rt.maxOccupancy ?? (rt as any).maxGuests ?? 2,
      })),
    };
  ...
  update: async (id: string, data: UpdateHotelRequest): Promise<Hotel | null> => {
    ...
    const roomTypes = data.roomTypes
      ? data.roomTypes.map((rt, idx) => ({
          ...
          stock: rt.stock ?? hotels[index].roomTypes[idx]?.stock ?? 10,
          status: rt.status ?? hotels[index].roomTypes[idx]?.status ?? 'available',
          bedType: rt.bedType ?? (rt as any).beds,
          maxOccupancy: rt.maxOccupancy ?? (rt as any).maxGuests,
        }))
